...
const QuestPage: React.FC = () => {
  const [quests, setQuests] = useState<{ title: string; content: string }[]>([])
  ...
  const handleAddQuest = (title: string, content: string) => {
    if (title.trim() !== '' && content.trim() !== '') {
      setQuests([...quests, { title, content }])
      ...
      <div className="ml-64 flex-1 p-8 overflow-y-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {quests.map((quest, index) => (
            <QuestBox key={index} title={quest.title} context={quest.content} />
          ))}
        </div>
      </div>
    </div>
